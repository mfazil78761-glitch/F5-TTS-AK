...
def push_database_updates_to_github(updated_db_dict):
    ...
    try:
        ...
        encoded_content = base64.b64encode(
            json.dumps({"users": updated_db_dict}, indent=2).encode("utf-8")
        ).decode("ascii")
...
with st.sidebar:
    ...
    if account_profile.get("is_admin", False):
        ...
        app_page_mode = st.radio(
            "🎛️ Navigation Controller",
            ["🎤 Reference Voice Cloning", "🔊 Text To Speech Engine"]
        )
...
if account_profile.get("is_admin", False):
    ...
    if app_page_mode == "🗃️ Active Users Registry":
        ...
        for u_name, u_info in list(user_db.items()):
            ...
            st.markdown(
                f"#### Profile Handle: `{u_name}` | Status: **{status_badge}**"
            )
            ...
            if st.button("💾 Save Changes", key=f"save_edit_{u_name}"):
                ...
                if extend_extra_days > 0:
                    ...
                    user_db[u_name]["expiry_timestamp"] = (
                        curr_exp + timedelta(days=int(extend_extra_days))
        ...
        with st.form("new_user_registration_form"):
            ...
            if create_user_btn and reg_user and reg_pass and reg_user not in user_db:
                calculated_expiry_timestamp = (
                    datetime.now() + timedelta(days=int(reg_days))
                ).strftime("%Y-%m-%d %H:%M:%S")
...
if time_delta_now.total_seconds() > 0:
    ...
    countdown_clock_string = (
        f"⏱️ Time Remaining: **{days} days {hours} hours "
        f"{minutes} minutes {seconds} seconds**"
    )
else:
    countdown_clock_string = (
        "🚨 Package Plan Validity Status: **Expired!**"
    )
...
<div style="background: linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%);
    ...
    <h3 style="margin:0; color:white;">
        ✨ Session Active: {active_username.upper()} Console
    </h3>
    <p style="margin:8px 0; font-size:17px; color:#6dd5ed;">
        {countdown_clock_string}
    </p>
    <p style="margin:0; font-size:14px; opacity:0.85;">
        Character Available Balance:
        <b>{account_profile['remaining_chars']:,} / {account_profile['total_limit']:,} Chars</b>
    </p>
</div>
""",
...
if app_page_mode == "🎤 Reference Voice Cloning":
    st.subheader("🎤 Reference Voice Cloning")
    st.info("Reference voice cloning interface yahan connect ki ja sakti hai.")

elif app_page_mode == "🔊 Text To Speech Engine":
    st.subheader("🔊 Text To Speech Engine")
    st.info("Text-to-speech engine interface yahan connect ki ja sakti hai.")
