...
st.set_page_config(page_title="F5-TTS Cloud Multi-Tenant SaaS", page_icon="👑", layout="wide")
...
if account_profile.get("is_admin", False):
    ...
    with tab_admin_list:
        ...
        for u_name, u_info in list(user_db.items()):
            ...
            col_u1, col_u2, col_u3, col_u4 = st.columns([1.5, 1.2, 1, 1])
            with col_u1:
                new_limit_chars = st.number_input(f"Characters Balance Remaining:", value=int(u_info["remaining_chars"]), key=f"char_{u_name}")
            with col_u2:
                extend_extra_days = st.number_input(f"Extend Plan Days (Add):", min_value=0, value=0, key=f"days_{u_name}")
            with col_u4:
                ...
                if st.button(btn_txt, key=f"rev_btn_{u_name}", use_container_width=True):
                    user_db[u_name]["is_revoked"] = not revoked_state_toggle
                    if push_database_updates_to_github(user_db):
                        st.success(f"Status changed successfully!")
                        st.rerun()
            with col_u3:
                st.write("<div style='margin-top:25px;'></div>", unsafe_allow_html=True)
                if st.button("💾 Save Changes", key=f"save_edit_{u_name}", use_container_width=True):
        ...
        if not active_users_exist:
            st.info("Filhal database registry mein koi regular user add nahi hai. Naya user add karne ke liye barabar waale tab par jayein.")
        ...
        with st.form("new_user_registration_form"):
            ...
            if create_user_btn:
                if not reg_user or not reg_pass:
                    ...
                    if push_database_updates_to_github(user_db):
                        st.success(f"🎉 User '{reg_user}' successfully deployed and auto-committed!")
                        st.rerun()
    st.stop()
